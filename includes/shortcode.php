	...
	<div class="vsp-wrapper" id="<?php echo esc_attr( $uid ); ?>" style="max-width:<?php echo esc_attr( $ancho ); ?>;" data-tiene-senas="<?php echo $tiene_senas ? '1' : '0'; ?>" data-tiene-subtitulos="<?php echo $tiene_subs ? '1' : '0'; ?>" data-main-type="<?php echo $main_vimeo ? 'vimeo' : 'file'; ?>"<?php echo $main_vimeo ? ' data-vimeo-id="' . esc_attr( $principal_vimeo ) . '"' : ''; ?><?php echo $tiene_senas ? ' data-senas-type="' . ( $senas_vimeo_on ? 'vimeo' : 'file' ) . '"' : ''; ?><?php echo $senas_vimeo_on ? ' data-senas-vimeo-id="' . esc_attr( $senas_vimeo ) . '"' : ''; ?><?php echo $tiene_subs ? ' data-subtitulos="' . esc_attr( $url_subs ) . '"' : ''; ?><?php echo $subs_inline !== '' ? ' data-subtitulos-data="' . esc_attr( $subs_inline ) . '"' : ''; ?><?php echo $subs_cues_b64 !== '' ? ' data-subtitulos-cues="' . esc_attr( $subs_cues_b64 ) . '"' : ''; ?>>
		...
		<div class="vsp-stage">
			...
			<div class="vsp-loading" aria-live="polite" aria-busy="true">
				...
				<p class="vsp-loading-text"><?php esc_html_e( 'Loading player…', 'arielhf-videopip-overlay' ); ?></p>
			</div>
			...
			<div class="vsp-signs-wrap" aria-hidden="false" style="display:block;">
				...
				<video class="vsp-signs"
					...
					muted></video>
				<?php endif; ?>
				<div class="vsp-signs-hit" aria-hidden="true"></div>
				<button type="button" class="vsp-signs-resize" aria-label="<?php esc_attr_e( 'Cambiar tamaño', 'arielhf-videopip-overlay' ); ?>" title="<?php esc_attr_e( 'Cambiar tamaño', 'arielhf-videopip-overlay' ); ?>"></button>
				<div class="vsp-signs-dragbar" title="<?php esc_attr_e( 'Arrastrar para mover', 'arielhf-videopip-overlay' ); ?>">
					<span class="vsp-signs-dragbar-grip" aria-hidden="true"></span>
					<span class="vsp-signs-dragbar-label"><?php esc_html_e( 'Arrastrar para mover', 'arielhf-videopip-overlay' ); ?></span>
				</div>
			</div>
			<?php endif; ?>

			<div class="vsp-overlay-play" role="button" tabindex="0" aria-label="<?php esc_attr_e( 'Play video', 'arielhf-videopip-overlay' ); ?>">
			...
			<div class="vsp-controls" role="group" aria-label="<?php esc_attr_e( 'Player controls', 'arielhf-videopip-overlay' ); ?>">

				<div class="vsp-progress-row">
					<span class="vsp-time-cur">0:00</span>
					<div class="vsp-progress-bg" role="slider" aria-label="<?php esc_attr_e( 'Progress', 'arielhf-videopip-overlay' ); ?>" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0" tabindex="0">
				...
				<div class="vsp-btn-row">
					...
					<button type="button" class="vsp-btn-play" aria-label="<?php esc_attr_e( 'Play', 'arielhf-videopip-overlay' ); ?>">
					...
					<div class="vsp-vol-group">
						<button type="button" class="vsp-btn-mute" aria-label="<?php esc_attr_e( 'Mute', 'arielhf-videopip-overlay' ); ?>">
						...
						<span class="vsp-vol-slot">
							<input class="vsp-vol" type="range" min="0" max="1" step="0.05" value="1" aria-label="<?php esc_attr_e( 'Volume', 'arielhf-videopip-overlay' ); ?>">
						</span>
					</div>
					...
					<?php if ( $tiene_subs ) : ?>
					<button type="button" class="vsp-btn-subtitles" aria-pressed="true" aria-label="<?php esc_attr_e( 'Desactivar subtítulos', 'arielhf-videopip-overlay' ); ?>">
						<span class="vsp-subtitles-label"><?php esc_html_e( 'Desactivar subtítulos', 'arielhf-videopip-overlay' ); ?></span>
					</button>
					<?php endif; ?>

					<?php if ( $tiene_senas ) : ?>
					<button type="button" class="vsp-btn-signs" aria-pressed="true" aria-label="<?php esc_attr_e( 'Desactivar lengua de señas', 'arielhf-videopip-overlay' ); ?>">
						<span class="vsp-signs-label"><?php esc_html_e( 'Desactivar lengua de señas', 'arielhf-videopip-overlay' ); ?></span>
					</button>
					<?php endif; ?>

					<button type="button" class="vsp-btn-fs" aria-label="<?php esc_attr_e( 'Fullscreen', 'arielhf-videopip-overlay' ); ?>">
